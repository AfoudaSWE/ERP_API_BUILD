...
export default function ExpensesPage() {
  ...
  const submit=async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);try{await apiRequest('/finance/expenses',{method:'POST',body:JSON.stringify({description:form.get('description'),amount:Number(form.get('amount')).toFixed(2),taxAmount:Number(form.get('tax')).toFixed(2),expenseDate:form.get('date'),paymentMethod:form.get('method')})});setShow(false);await load();}catch(c){setError(c instanceof Error?c.message:'Failed');}};
