...
export async function createAccountingFixture() {
  const tenantId=randomUUID(),companyId=randomUUID(),userId=randomUUID();
  await pool.query(`INSERT INTO tenants(id,name,slug)VALUES($1,'Accounting Test',$2)`,[tenantId,`accounting-${tenantId}`]);
  await pool.query(`INSERT INTO companies(id,tenant_id,name)VALUES($1,$2,'Accounting Test')`,[companyId,tenantId]);
