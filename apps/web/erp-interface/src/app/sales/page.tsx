...
export default function SalesPage() {
  ...
  const [actionMode, setActionMode] = useState<"payment" | "return" | null>(
    null,
  );
  ...
  const openReturn = async (invoice: SalesInvoice) => {
    setActionError("");
    try {
      const detail = await apiGet<SalesInvoice>(
        `/sales/invoices/${invoice.id}`,
      );
      setActionInvoice(detail);
      setReturnItemId(detail.items[0]?.id ?? "");
      setActionMode("return");
    } catch (cause) {
      setActionError(
        cause instanceof Error ? cause.message : "Unable to load invoice",
      );
    }
  };
  const submitAction = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (!actionInvoice || !actionMode) return;
    ...
    try {
      const path =
        actionMode === "payment"
          ? `/sales/invoices/${actionInvoice.id}/payments`
          : `/sales/invoices/${actionInvoice.id}/returns`;
      const body =
        actionMode === "payment"
          ? {
  ...
  const filteredInvoices = salesInvoices.filter((invoice) => {
    ...
    const matchesPayment =
      !paymentFilter || invoice.paymentStatus === paymentFilter;

    return matchesSearch && matchesStatus && matchesPayment;
  });

  const formatPrice = (value: number) =>
    formatCurrency(value, "EGP", locale === "ar" ? "ar-EG" : "en-EG");
  ...
  const stats = {
    ...
    pendingAmount: salesInvoices.reduce(
  ...
  return (
    <AppLayout>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div className="stat-card">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="stat-label">
                {locale === "ar" ? "المدفوع" : "Collected"}
              </p>
              <p className="stat-value text-lg text-success-600">
        ...
        <div className="stat-card">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="stat-label">
                {locale === "ar" ? "المتبقي" : "Pending"}
              </p>
              <p className="stat-value text-lg text-warning-600">
        ...
        <div className="p-4">
          <div className="flex flex-col md:flex-row gap-4">
            ...
            <div className="w-full md:w-40">
              ...
              >
                <option value="">
                  {locale === "ar" ? "كل الدفع" : "All Payment"}
                </option>
                <option value="paid">
                  {locale === "ar" ? "مدفوع" : "Paid"}
                </option>
                        ...
                        >
                          {invoice.source === "ecommerce"
                            ? locale === "ar"
                              ? "المتجر"
                              : "E-commerce"
                            : (invoice.source ?? "ERP").toUpperCase()}
                        </span>
                      </div>
                    </td>
                    <td>
                      {customer ? (
                        ...
                        <span className="text-navy-500">
                          {invoice.customerName ||
                            (locale === "ar" ? "عميل نقدي" : "Cash Customer")}
                        </span>
                      )}
                    </td>
                    <td>
                    ...
                    <td>
                      <div className="flex items-center justify-end gap-1">
                        ...
                        {can("sales.refund") && (
                          <button
                            type="button"
                            onClick={() => void openReturn(invoice)}
                            className="btn btn-ghost btn-icon btn-sm"
                            title={locale === "ar" ? "مرتجع" : "Return"}
                          >
      ...
      {actionMode && actionInvoice && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-navy-950/60 p-4">
          <form onSubmit={submitAction} className="card w-full max-w-md p-5">
            ...
            {actionMode === "payment" ? (
              <>
                ...
                <input
                  ...
                  type="number"
                  min="0.01"
                  max={actionInvoice.remainingAmount}
                  step="0.01"
                  required
                  className="input mb-3"
                />
            ...
            <div className="mt-5 flex justify-end gap-2">
              ...
              <button disabled={actionSaving} className="btn btn-primary">
                {actionSaving
                  ? locale === "ar"
                    ? "جارٍ الحفظ…"
                    : "Saving…"
