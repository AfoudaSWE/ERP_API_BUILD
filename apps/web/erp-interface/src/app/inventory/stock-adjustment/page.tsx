...
export default function StockAdjustmentPage() {
  ...
  async function submit(e: FormEvent<HTMLFormElement>) {
    ...
    const magnitude = String(data.get("quantity"));
    const quantity = direction === "decrease" ? `-${magnitude}` : magnitude;
    try {
      ...
      form.reset();
      setMessage(ar ? "تم تسجيل التسوية بنجاح." : "Adjustment posted successfully.");
    } catch (err) {
    ...
    <AppLayout>
      <main className="mx-auto max-w-xl space-y-6 p-4 sm:p-6">
        <header className="flex items-center gap-2">
          <ClipboardEdit className="h-6 w-6 text-primary-600" />
          <div>
            <h1 className="text-2xl font-bold">{ar ? "تسوية المخزون" : "Stock Adjustment"}</h1>
            <p className="text-sm text-navy-500">{ar ? "زيادة أو تقليل رصيد صنف في مخزن معيّن" : "Increase or decrease a product's balance in a warehouse"}</p>
          </div>
        </header>
        ...
        <form onSubmit={(e) => void submit(e)} className="card space-y-4 p-6">
          <label className="block text-sm">
            {ar ? "المخزن" : "Warehouse"}
            <select required name="warehouseId" className="select mt-1 w-full">
          ...
          <div className="grid grid-cols-2 gap-4">
            <label className="block text-sm">
              {ar ? "نوع التسوية" : "Direction"}
              <select name="direction" className="select mt-1 w-full">
                <option value="increase">{ar ? "زيادة" : "Increase"}</option>
                <option value="decrease">{ar ? "نقص" : "Decrease"}</option>
              </select>
            </label>
            <label className="block text-sm">
              {ar ? "الكمية" : "Quantity"}
          ...
          <label className="block text-sm">
            {ar ? "السبب" : "Reason"}
            <textarea required name="reason" minLength={3} maxLength={500} rows={3} className="input mt-1 w-full" placeholder={ar ? "مثال: جرد فعلي، تلف، خطأ إدخال" : "e.g. physical count, damage, entry correction"} />
          </label>
          <button disabled={busy} className="btn btn-primary w-full justify-center">
            {busy && <Loader2 className="h-4 w-4 animate-spin" />}
            {ar ? "تسجيل التسوية" : "Post adjustment"}
          </button>
        </form>
      </main>
    </AppLayout>
