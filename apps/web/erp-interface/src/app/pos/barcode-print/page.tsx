...
import { Barcode as BarcodeIcon, Minus, Plus, Printer, Search } from 'lucide-react';
...
export default function BarcodePrintPage() {
  ...
  const printable = useMemo(() => products.filter((product) => product.isActive && product.barcode), [products]);
  ...
  return (
    <AppLayout>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        <div>
          <h1 className="text-2xl font-bold text-navy-900 dark:text-white">{ar ? 'طباعة الباركود' : 'Barcode Print'}</h1>
      ...
      <div className="print:hidden">
        ...
        <div className="card overflow-x-auto">
          <table className="table min-w-[600px]">
            <thead><tr><th>{ar ? 'المنتج' : 'Product'}</th><th>{ar ? 'الباركود' : 'Barcode'}</th><th>{ar ? 'السعر' : 'Price'}</th><th className="text-end">{ar ? 'عدد الملصقات' : 'Labels'}</th></tr></thead>
            <tbody>
              {filtered.map((product) => (
                <tr key={product.id}>
                  <td className="font-medium">{ar ? product.nameAr || product.name : product.name}</td>
                  <td className="tabular-nums text-navy-500">{product.barcode}</td>
          ...
          {!filtered.length && <div className="empty-state"><BarcodeIcon className="empty-state-icon" /><p>{ar ? 'لا توجد منتجات لها باركود' : 'No products with a barcode set'}</p></div>}
        </div>
      </div>

      {labels.length > 0 && (
        <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4 print:mt-0 print:grid-cols-3">
          {labels.map(({ product, key }) => <BarcodeLabel key={key} name={ar ? product.nameAr || product.name : product.name} value={product.barcode as string} price={formatPrice(product.sellingPrice)} />)}
        </div>
      )}
    </AppLayout>
  );
...
function BarcodeLabel({ name, value, price }: { name: string; value: string; price: string }) {
  const svgRef = useRef<SVGSVGElement>(null);

  useEffect(() => {
    if (!svgRef.current) return;
    try { JsBarcode(svgRef.current, value, { format: 'CODE128', width: 1.6, height: 40, fontSize: 12, margin: 4, displayValue: true }); }
    catch { /* invalid barcode value for CODE128, skip rendering */ }
  }, [value]);
